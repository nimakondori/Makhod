...
def count_down(tz: timezone) -> dict:
    ...
    tz = timezone(tz)
    print (tz)
    anniversary = datetime(datetime.now(tz).year, ANNIVERSARY_MONTH, ANNIVERSARY_DAY, 10, 50,0,0)
    today = datetime.now()
    ...
    if int(total_days[0]) == -1:
       anniversary = datetime(datetime.now().year + 1, ANNIVERSARY_MONTH, ANNIVERSARY_DAY, 10, 40,0,0)
       timedelta = anniversary - today
       seconds_in_day = 24 * 60 * 60
       print("anniversary updated ", anniversary)
       duration_in_s = timedelta.total_seconds()
       total_days = divmod(duration_in_s, 86400)  # Get days (without [0]!)
